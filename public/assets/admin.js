...
function _bpRenderPreview(){
  ...
  const ingsHtml=bp.ingredients.length?bp.ingredients.map(ing=>{const modsStr=(ing.modifiers||[]).map(m=>`${esc(m.property||'')}: ${m.modifierAtStart}→${m.modifierAtEnd}`).join(', ');return`<div class="bp-ing-row"><div style="flex:1;font-size:0.88rem;color:var(--text2);">${esc(ing.name)}</div><span style="font-size:0.78rem;color:var(--muted);">×${ing.quantity}</span>${modsStr?`<span style="font-size:0.68rem;color:var(--muted2);margin-left:8px;">${modsStr}</span>`:''}</div>`;}).join(''):'<div style="font-size:0.82rem;color:var(--muted);">Sem ingredientes.</div>';
